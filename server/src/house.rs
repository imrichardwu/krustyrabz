...
use poker_core::{
    ActionRequest, AddChipsRequest, AddChipsResponse, CreateGameRequest, GameListResponse,
    GameResponse, GameStateUpdate, GameSummary, GameType, HouseRules, JoinGameRequest, PlayerInfo,
    PlayerStats, ServerResponse, WithdrawChipsRequest, WithdrawChipsResponse,
};
...
impl House {
    ...
    pub fn create_new_game(&self, game_type: GameType) -> Result<Game, GameError> {
        match game_type {
    ...
    /// Adds a player to a specific game (if `game_id` is given) or to the first
    /// available game of the requested type. Returns the game_id on success.
    pub fn lock_and_add(
        &self,
        player: Player,
        ...
        match game_id {
            Some(id) => {
                ...
                match games.get_mut(&id) {
                    ...
                    None => Err(GameError::GameNotFound(id.clone())),
                }
            }
            None => {
    ...
    /// Removes a game from the house.
    pub fn remove_game(&self, game_id: &str, kill_stream: Option<bool>) -> Result<(), GameError> {
        let mut games = self.live_games.lock().unwrap();
        games
            .remove(game_id)
            .ok_or_else(|| GameError::GameRemoveFailed(game_id.to_string()))?;
        drop(games);
    ...
    /// Removes a player from a game, deleting the game if it becomes empty.
    pub fn remove_player(&self, game_id: &str, player_id: Uuid) -> Result<(), GameError> {
        let mut games = self.live_games.lock().unwrap();
        match games.get_mut(game_id) {
            ...
            None => Err(GameError::GameNotFound(game_id.to_string())),
        }
    }

    /// Finds an open game for the player, or creates a new one if none is available.
    /// Returns the game_id on success.
    pub fn find_or_create_game(
        &self,
        player: Player,
        game_type: GameType,
    ) -> Result<String, GameError> {
...
pub async fn create_game(
    ...
    match new_game.add_player(player) {
        ...
        Err(e) => Json(GameResponse::error(format!(
    ...
    match house.remove_player(&game_id, player_id) {
        Ok(_) => {
            let game_state = {
                ...
                games
                    .get(&game_id)
                    .map(|game| build_game_state_update(game, None))
            }; // games lock released
...
pub async fn start_hand(
    ...
    let outcome: Result<GameStateUpdate, GameError> = {
        let mut games = house.live_games.lock().unwrap();
        ...
        game.start_hand()
            .map(|()| build_game_state_update(game, Some(&player_id_str)))
    }; // games lock released
    ...
    match outcome {
        ...
        Err(e) => Json(GameResponse::error(e.to_string())),
    }
}
...
pub async fn perform_action(
    ...
    if inner.game_id != game_id {
        return Json(GameResponse::error(
            "game_id in URL and body must match".to_string(),
        ));
    }
    ...
    let outcome: Result<GameStateUpdate, GameError> = {
        ...
        let result = match game.get_game_type() {
            poker_core::GameType::FiveCardDraw => match &inner.action {
                ...
                GameAction::AllIn => Err(GameError::AllInNotSupportedFiveCardDraw),
            },
        ...
        if before_showdown.is_none() && after_showdown.is_some() {
            if let Some(ref showdown_data) = after_showdown {
                ...
                for (username, amount) in showdown_data {
                    if let Some(player) =
                        game.get_players().iter().find(|p| &p.username == username)
                    {
    ...
    if let Some(winners) = showdown_results {
        for (winner_id, winnings) in winners {
            if let Err(e) = repo
                .update_user_token_balance(winner_id, winnings as f64)
                .await
            {
...
pub async fn get_stats(player_id: String, repo: &State<Repository>) -> Json<PlayerStats> {
...
/// Adds chips to a player's account, validating against the player's credit limit.
...
/// Withdraws chips from a player's account, validating sufficient balance.
#[post(
    "/players/<player_id>/withdrawchips",
    format = "json",
    data = "<request>"
)]
...
pub async fn sit_out(
    ...
    match games.get_mut(&game_id) {
        Some(game) => match game.sit_out_player(player_uuid) {
            ...
            Err(e) => Json(ServerResponse::error(e.to_string())),
        },
            ...
            match game_type {
                "FiveCardDraw" => {
                    if game.get_game_type() == GameType::FiveCardDraw {
                        return Json(ServerResponse::success(
                            "Nothing to do; game is already of the selected type",
                        ));
                    }
                    let mut new_fcd = match house.create_new_game(GameType::FiveCardDraw) {
                        Ok(game) => game,
                        Err(e) => {
                            return Json(ServerResponse::error(format!(
                                "Failed to change game variant: {}",
                                e
                            )));
                        }
                    };
...
async fn get_player_from_db(player_id: &str) -> Result<Player, GameError> {
    let player_uuid =
        Uuid::parse_str(player_id).map_err(|e| GameError::InvalidUuid(e.to_string()))?;

    let repo = Repository::new()
        .await
        .map_err(|e| GameError::RepositoryCreate(e.to_string()))?;

    let model = repo
        .get_user_by_id(player_uuid)
        .await
        .map_err(|e| GameError::UserLookup(e.to_string()))?;
...
fn handle_fold(game: &mut Game, player_id: Uuid) -> Result<(), GameError> {
    match game {
        Game::FiveCardDraw(g) => match g.betting_round {
            ServerBettingRound::PreDeal => Err(GameError::GameNotStarted),
            ...
            ServerBettingRound::Drawing => Err(GameError::CannotFoldInDrawPhase),
            _ => Err(GameError::WrongPhase),
        },
        _ => Err(GameError::OnlyFiveCardDrawSupported),
    }
}

/// Handles a check action (PreDraw or PostDraw betting).
fn handle_check(game: &mut Game, player_id: Uuid) -> Result<(), GameError> {
    match game {
        Game::FiveCardDraw(g) => match g.betting_round {
            ServerBettingRound::PreDeal => Err(GameError::GameNotStarted),
            ...
            ServerBettingRound::Drawing => Err(GameError::CannotCheckInDrawPhase),
            _ => Err(GameError::WrongPhase),
        },
        _ => Err(GameError::OnlyFiveCardDrawSupported),
    }
...
fn handle_call(game: &mut Game, player_id: Uuid) -> Result<(), GameError> {
    match game {
        Game::FiveCardDraw(g) => match g.betting_round {
            ServerBettingRound::PreDeal => Err(GameError::GameNotStarted),
            ...
            ServerBettingRound::Drawing => Err(GameError::CannotCallInDrawPhase),
            _ => Err(GameError::WrongPhase),
        },
        _ => Err(GameError::OnlyFiveCardDrawSupported),
    }
...
fn handle_bet(game: &mut Game, player_id: Uuid, amount: u32) -> Result<(), GameError> {
    match game {
        Game::FiveCardDraw(g) => match g.betting_round {
            ServerBettingRound::PreDeal => Err(GameError::GameNotStarted),
            ServerBettingRound::PreDraw => g.predraw_betting(player_id, GameAction::Bet { amount }),
            ServerBettingRound::PostDraw => {
                g.postdraw_betting(player_id, GameAction::Bet { amount })
            }
            ServerBettingRound::Drawing => Err(GameError::CannotBetInDrawPhase),
            _ => Err(GameError::WrongPhase),
        },
        _ => Err(GameError::OnlyFiveCardDrawSupported),
    }
...
fn handle_raise(game: &mut Game, player_id: Uuid, amount: u32) -> Result<(), GameError> {
    match game {
        Game::FiveCardDraw(g) => match g.betting_round {
            ServerBettingRound::PreDeal => Err(GameError::GameNotStarted),
            ...
            ServerBettingRound::Drawing => Err(GameError::CannotRaiseInDrawPhase),
            _ => Err(GameError::WrongPhase),
        },
        _ => Err(GameError::OnlyFiveCardDrawSupported),
    }
...
fn handle_pass(game: &mut Game, player_id: Uuid) -> Result<(), GameError> {
    match game {
        Game::FiveCardDraw(g) => match g.betting_round {
            ServerBettingRound::PreDeal => Err(GameError::GameNotStarted),
            ...
            ServerBettingRound::Drawing => Err(GameError::CannotPassInDrawPhase),
            _ => Err(GameError::WrongPhase),
        },
        _ => Err(GameError::OnlyFiveCardDrawSupported),
    }
