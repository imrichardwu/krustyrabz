...
fn chips_form(message: Option<(&str, bool)>) -> Markup {
    html! {
        div class="w-full max-w-md" {
            ...
            form hx-post="/chips" hx-target="#main-content" hx-swap="innerHTML" class="flex flex-col gap-5" {
                div {
                ...
                div {
                    label for="chips-amount" class="block text-xs font-semibold uppercase tracking-widest mb-1.5" style="color:#7a8fa6;" { "Custom Amount" }
                ...
                p class="text-xs text-center" style="color:#2d3a4a;" { "Cannot add chips during an active game." }
            }
        }
    }
}
...
pub async fn add_chips_post(
    ...
    match client.add_chips(&user_id, req.amount).await {
        Ok(_) => Ok(chips_form(Some((&format!("Successfully added {} chips to your account!", req.amount), true)))),
        Err(_) => Ok(chips_form(Some(("Failed to add chips. Please try again.", false)))),
    }
}

pub fn routes() -> Vec<rocket::Route> {
    rocket::routes![add_chips, add_chips_post]
}
